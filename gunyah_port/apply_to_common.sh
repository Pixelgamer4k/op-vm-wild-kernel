...
if [[ -f "$DEFCONFIG" ]]; then
  ...
  # Force-disable legacy OEM Gunyah bits on GKI (headers/traces/APIs conflict)
  for key in \
    CONFIG_GH_SECURE_VM_LOADER CONFIG_GH_PROXY_SCHED CONFIG_GH_VIRT_WATCHDOG \
    CONFIG_GUNYAH_DRIVERS CONFIG_GH_CTRL CONFIG_GH_DBL CONFIG_GH_MSGQ \
    CONFIG_GH_RM_DRV CONFIG_GH_IRQ_LEND CONFIG_GH_MEM_NOTIFIER CONFIG_GH_GUEST_POPS \
    CONFIG_GUNYAH_QCOM_PLATFORM \
    CONFIG_GUNYAH CONFIG_GUNYAH_PLATFORM_HOOKS CONFIG_GUNYAH_VCPU \
    CONFIG_GUNYAH_IRQFD CONFIG_GUNYAH_IOEVENTFD
  do
    sed -i "/^# ${key} is not set$/d; /^${key}=/d" "$DEFCONFIG"
