...
/* 5.15 mailbox lacks mbox_bind_client(); mirror its bind logic locally. */
static int gh_mbox_bind_client(struct mbox_chan *chan, struct mbox_client *cl)
{
	...
	if (chan->mbox->dev->driver)
		owner = chan->mbox->dev->driver->owner;

	if (chan->cl || !try_module_get(owner)) {
		 ...
		 struct gh_resource *tx_ghrsc, struct gh_resource *rx_ghrsc)
{
	...
	ret = gh_mbox_bind_client(gh_msgq_chan(msgq), cl);
	if (ret)
		goto err_mbox;
